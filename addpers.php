...
if(!isset($_REQUEST['action']) || $_REQUEST['action']=='')$_REQUEST['action']='add';
echo PrintHead('Персонал','Добавление нового сотрудника');
/*
...
{
    if ($ext == 'int')$_REQUEST['ext_int']="";
    if ($ext == 'text')$_REQUEST['ext_text']="";
  $_REQUEST['tabnum']="";
  $_REQUEST['date_in']=date("d.m.Y");
  $_REQUEST['date_out']='';
  $_REQUEST['family']="";
  $_REQUEST['fname']="";
  $_REQUEST['secname']="";
  $_REQUEST['position']="";
  $_REQUEST['id_dept']="";
  $_REQUEST['graph_name']="NULL";
// 11.10.2010   
  $_REQUEST['graph_offset'] = 0;
  ...
  $_REQUEST['pxcodenum']=NULL;

  $_REQUEST['pxdatein']=date('d.m.Y');//--'';
  $_REQUEST['pxdateout']='';
  $_REQUEST['pincod']='';
  $_REQUEST['status']='';
  $_REQUEST['id_dopusk']=0;
  $_REQUEST['p_id_dopusk']=0;
  //питание
  $_REQUEST['breakfast'] = 0;
  $_REQUEST['din'] = 0;
  $_REQUEST['supper'] = 0;


 //$PATHPHOTO.=$_REQUEST['PHOTO'];
    $fhoto=$PATHPHOTO.$_REQUEST['photoname'];
    $fhoto=str_replace("\\","/",$PATHPHOTO.$_REQUEST['photoname']);
    $foto_isx = $_REQUEST['photoname'];
}
...
$BODY.='<div id="mask" style="position: absolute;
   ...
   $BODY.='<form name="aepers" action="save_pers_data.php?act=insert" method="POST">';
   $BODY.='<input  type="hidden" name="id_pers" value="0">';
   ...
   <td><input type="text" name="tabnum" autofocus value="'.$_REQUEST['tabnum'].'" size=20 class="input"></td>';
   ...
   <td><input type="text" name="family" required value="'.$_REQUEST['family'].'" size="20" class="input" ></td>';
   $BODY.='</tr>';
   $BODY.='<tr>';
   ...
   $BODY.= '<input type="text" id ="deptName" name="deptName" value="" size="20" class="input">&nbsp;';
   ...
   $BODY.= '<input type="hidden" id ="id_dept" name="id_dept" value="'.$_REQUEST['id_dept'].'"></td>';
   $BODY.='</td>';
...
$BODY.= '</select>
    <input type="button" value="просмотр" name="showgraph" class="sbutton" onclick=\'showGraphInfo(document.aepers.graph,event)\' />
    ...
    $BODY.='<tr><td><p class="text">Смещение в графике:</p></td><td><input type="text" name="graph_offset" value="0" size="2" class="input"></td></tr>';
   $disable = '';
   $checked = '';
   ...
   $BODY.= '</select>
   <input type="hidden" name="id_zone" value="'.$_REQUEST['id_zone'].'">
...
//// ВНИМАНИЕ!! Длина кода пропуска железно забита в 16 символов!!
   ...
   <td><input type="text" name="pxcodenum" value="'.$_REQUEST['pxcodenum'].'" size=20 class="input" maxlength="16">
       ...
       <input type="hidden" name="pxcodenum_old" value="" />
   </td>';
   $BODY.='</tr>';
   //Реквизиты пропуска
   $BODY.='<tr><td colspan="2">';
     ...
     $BODY.='<tr><td><span class="text">Pin код</span></td>
            ...
            <tr><td><span class="text">Коментарий</span></td>
                <td><input type="text" name="comment" size="20" value=""  class="input"></td>
            </tr>';
     $BODY.='</tr>';
     $BODY.='<tr><td colspan="2"><p class="text">Статус пропуска</p>
   ...
   $BODY.='<td><input type="button" value="добавить" name="addbt" onClick=\'AddEdPers(document.aepers,"insert")\' class="sbutton" /></td>';
